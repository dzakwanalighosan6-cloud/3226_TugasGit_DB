...
<div class="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm max-w-2xl">
    <form action="{{ route('admin.categories.update', $category->id) }}" method="POST" class="space-y-6">
        ...
        <div>
            ...
            <p class="text-xs text-slate-400 mt-2">Slug akan otomatis diperbarui mengikuti nama.</p>
        </div>
        <div class="pt-4 flex justify-end gap-4 border-t border-slate-100">
            ...
            <button type="submit" class="px-8 py-4 bg-indigo-600 text-white rounded-2xl font-bold shadow-lg shadow-indigo-100 hover:bg-indigo-700 transition">Update Kategori</button>
        </div>
    </form>
</div>
@endsection